...
export class RecallPage {
  ...
  private readonly orderDetailsMoreButton: Locator;
  ...
  constructor(private readonly page: Page) {
    ...
    this.orderDetailsPriceSummaryToggle = this.orderDetailsDialog
      ...
      .or(this.orderDetailsDialog.getByRole('button', { name: /Total\s*\$[\d,.]+/ }))
      .first();
    this.orderDetailsPriceSummaryDetailsContainer =
      this.orderDetailsPriceSummaryToggle.locator('xpath=following-sibling::*[1]');
    this.orderDetailsMoreButton = this.orderDetailsDialog
      .locator(
        [
          '[data-test-id="recall2-order-detail-more"]',
          '[data-testid="recall2-order-detail-more"]',
        ].join(', '),
      )
      .or(this.orderDetailsDialog.getByRole('button', { name: /\bMore\b/i }))
      .or(this.orderDetailsDialog.locator('button').filter({ hasText: /^More$/ }))
      .or(
        this.page.locator(
          [
            '#odsmymoreicon',
      ...
      .or(this.page.getByRole('button', { name: /(^|\s)More$/i }))
      .first();
    this.voidOrderButton = this.page
      .locator(
        [
          '#voidod',
          '[data-test-id="recall2-order-detail-void"]',
          '[data-testid="recall2-order-detail-void"]',
        ].join(', '),
      )
      .or(this.page.getByRole('button', { name: /^Void$/i }))
      .or(this.page.getByRole('menuitem', { name: /^Void$/i }))
      .first();
    this.voidAllMoreButton = this.page
      .locator(
        [
          '#pvoidod',
          '[data-test-id="recall2-order-detail-void-all"]',
          '[data-testid="recall2-order-detail-void-all"]',
        ].join(', '),
      )
      .or(this.page.getByRole('button', { name: /Void All/i }))
      .or(this.page.getByRole('menuitem', { name: /Void All/i }))
      .first();
    this.voidDialog = this.page
      .locator('[role="dialog"]:visible')
      .filter({ has: this.page.getByText('Void Reason', { exact: true }) })
      .last();
    this.voidRestoreInventoryCheckbox = this.voidDialog
      .locator(
        [
          '[data-test-id="shared-void-dialog-restore-inventory-checkbox"]',
          '[data-testid="shared-void-dialog-restore-inventory-checkbox"]',
          'input[aria-label="Restore inventory"]',
          '.void-checkbox',
        ].join(', '),
      )
      .first();
    this.voidNoteInput = this.voidDialog
      .getByRole('textbox', { name: /^Note$/i })
      .or(this.voidDialog.locator('#void-note-input:visible'))
      .or(this.page.locator('#void-note-input:visible'))
      .first();
    this.voidSubmitButton = this.voidDialog
      .getByRole('button', { name: /^Void$/i })
      .or(this.voidDialog.locator('#void-submit:visible'))
      .or(this.page.locator('#void-submit:visible'))
      .first();
  ...
  async voidCurrentOrder(options: { restoreInventory?: boolean; reason?: string } = {}): Promise<void> {
    ...
    await expect(this.orderDetailsDialog).toBeHidden({ timeout: 15_000 }).catch(() => undefined);
  }
  ...
  async exitRecall(): Promise<void> {
    ...
    const exitButtonState = await waitUntil(
      async () => ({
        ...
        orderDetailsVisible: await this.orderDetailsDialog.isVisible().catch(() => false),
        url: this.page.url(),
      }),
  ...
  private async openVoidActionFromOrderDetails(): Promise<void> {
    ...
    await this.orderDetailsMoreButton.click();
  ...
  private async dismissOrderDetailsDialogIfNeeded(): Promise<void> {
    if (!(await this.orderDetailsDialog.isVisible().catch(() => false))) {
      return;
    }
