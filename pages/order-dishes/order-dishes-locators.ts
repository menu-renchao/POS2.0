...
export class OrderDishesLocators {
  ...
  readonly changePriceInput: Locator;
  readonly currencyKeypadButton: (keypadInput: string) => Locator;
  readonly taxExemptButton: Locator;
  readonly taxExemptionDialog: Locator;
  readonly taxExemptionChoiceButton: (taxExempt: boolean) => Locator;
  ...
  readonly countDialogClearButton: Locator;
  readonly countDialogKeyButton: (key: string) => Locator;
  ...
  readonly searchMenuResultByName: (dishName: string) => Locator;
  ...
  readonly categoryOptionButton: (optionPattern: RegExp) => Locator;
  readonly categorySubOptionButton: (optionPattern: RegExp) => Locator;
  ...
  readonly comboSectionItemButton: (sectionName: string, dishName: string) => Locator;
  readonly comboSectionItemPlusButton: (sectionName: string, dishName: string) => Locator;
  ...
  readonly keepSplitOrderButton: Locator;
  readonly sharedSeatButton: Locator;
  readonly seatButton: (seatNumber: number) => Locator;
  ...
  constructor(page: Page) {
    this.page = page;
    this.appFrame = page.frameLocator('iframe[data-wujie-id="orderDishes"]');
    ...
    this.guestCountInput = this.guestCountDialog.locator('input').first();
    ...
    this.headerRecallButton = this.page.getByRole('button', { name: /Recall/ }).first();
    this.sendButton = this.page.getByTestId('bottom-button-sendOrderBtn');
    ...
    this.addLineButton = this.page.getByTestId('action-rail-button-addline');
    this.selectedDishAddButton = this.page.getByTestId('action-rail-button-add1');
    this.changePriceButton = this.page.getByTestId('action-rail-button-chgPrc');
    this.changePriceClearButton = this.page.getByTestId('preset-currency-keypad-input-clear');
    const changePriceDialog = this.page.getByRole('dialog', {
      name: 'Price',
      exact: true,
    });
    this.changePriceConfirmButton = changePriceDialog
      .getByRole('button', { name: 'Confirm', exact: true });
    this.changePriceInput = changePriceDialog.getByTestId(
      'preset-currency-keypad-input-input',
    );
    this.currencyKeypadButton = (keypadInput: string) =>
      this.page.getByTestId(
        keypadInput === 'double-zero'
          ? 'preset-currency-keypad-input-double-zero'
          : `preset-currency-keypad-input-number-${keypadInput}`,
      );
    this.taxExemptButton = this.page.getByTestId('action-rail-button-taxExempt');
    this.taxExemptionDialog = this.page
      .getByRole('dialog')
      .filter({ has: this.page.getByText(/Tax exemption or not\?/i) });
    this.taxExemptionChoiceButton = (taxExempt: boolean) =>
      this.taxExemptionDialog.getByRole('button', {
        name: taxExempt ? 'Exempt' : 'Taxes',
        exact: true,
      });
    ...
    this.countButton = this.page.getByTestId('action-rail-button-chgNum');
    ...
    this.countDialog = this.page.getByTestId('dish-count-modal');
    this.countDialogInput = this.countDialog.getByRole('textbox');
    this.countDialogClearButton = this.countDialog.getByRole('button', {
      name: 'C',
      exact: true,
    });
    this.countDialogKeyButton = (key: string) =>
      this.countDialog.getByRole('button', { name: key, exact: true });
    ...
    this.searchMenuResultByName = (dishName: string) =>
      this.page.getByRole('button', { name: dishName, exact: true });
    ...
    this.categoryOptionPanel = this.page.getByTestId('item-option-panel');
    ...
    this.categoryOptionButton = (optionPattern: RegExp) =>
      this.categoryOptionGrid.getByRole('button', { name: optionPattern }).first();
    this.categorySubOptionButton = (optionPattern: RegExp) =>
    ...
    this.comboSectionItemButton = (sectionName: string, dishName: string) =>
      this.comboDialog
        .getByRole('region', { name: sectionName, exact: true })
        .getByRole('button', { name: dishName, exact: true });
    this.comboSectionItemPlusButton = (sectionName: string, dishName: string) =>
      this.comboSectionItemButton(sectionName, dishName).getByRole('button', {
        name: '+',
        exact: true,
      });
    ...
    this.cartBadge = this.page.getByTestId('cart-badge');
    this.priceSummaryToggle = this.page
      .locator('[data-test-id="shared-order-price-summary-toggle"]:visible')
      .last();
    this.priceSummaryDetailsContainer = this.priceSummaryToggle;
    this.priceSummaryTotalContainer = this.priceSummaryToggle;
    this.saveOrderButton = this.page.getByTestId('bottom-button-saveOrderBtn');
    this.headerMoreButton = this.page.getByTestId('icon-button-more');
    this.inventoryMenuItem = this.page.getByTestId('dropdown-item-inventory');
    this.reduceButton = this.page.getByRole('button', {
      name: 'Reduce',
      exact: true,
    });
    this.exitConfirmButton = this.page.locator('#exit-edit-submit');
    this.inventoryAlertItems = this.page.locator('#inventory-alert-dialog');
    this.moreActionButton = this.page.getByTestId('bottom-button-more');
    this.splitButton = this.page.getByRole('button', {
      name: 'Split',
      exact: true,
    });
    this.keepSplitOrderButton = this.page.getByTestId('button-keep');
    this.sharedSeatButton = this.page.getByRole('button', {
      name: 'Share For Whole Table',
      exact: true,
    });
    this.seatButton = (seatNumber: number) =>
      this.page.getByRole('button', { name: `Seat ${seatNumber}`, exact: true });
    this.modifyButton = this.page.getByRole('button', { name: /^Modify$/ }).first();
    this.modifyPanel = this.page
      .locator(
        '[data-testid="modify-panel"]',
      )
      .filter({
