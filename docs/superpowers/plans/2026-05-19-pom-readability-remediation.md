...
- [ ] **Step 1: 为首页入口、点单页加载、Recall 搜索与详情补齐最小行为护栏**
- [ ] **Step 2: 确保新增断言只锁定公开 page 行为，不把内部 locator 实现细节写死**
- [ ] **Step 3: 运行首页、点单、Recall 相关 e2e/smoke 用例，确认重构前基线为绿**
...
- [ ] **Step 1: 提供共享的页面作用域 helper，统一处理 iframe 与宿主页面互斥渲染，不再让每个按钮各自堆 `.or(...)`**
- [ ] **Step 2: 定义页面方法命名与返回值契约的共享类型，如入口枚举、同页动作/跨页动作约定**
- [ ] **Step 3: 先在 `HomePage`、`SelectTablePage`、`InventoryPage` 落地 typed entry 与共享作用域，作为后续大页重构模板**
- [ ] **Step 4: 回跑 `home-page-refresh.spec.ts`、`select-table-page.spec.ts` 与相关 smoke 用例，确认模板页改造稳定**
...
- [ ] **Step 1: 保留 `OrderDishesPage` 作为 facade，只承担公开 API 编排与跨区块衔接**
- [ ] **Step 2: 将菜单切换/点菜、改单、加费、价格与购物车读取分别下沉到 section/helper 文件，避免单文件继续承载全部细节**
- [ ] **Step 3: 清理 `OrderDishesPage` 中重复的 `resolveX()` 与 locator fallback 链，优先走共享作用域 helper 和单一 DOM 契约**
- [ ] **Step 4: 确保 `saveOrder()`、`sendOrder()`、`openPayment()`、`openSplitOrder()` 等跨页动作保留清晰返回值与最小后置条件**
- [ ] **Step 5: 回跑 `order-dishes-*.spec.ts`、`payment-page.spec.ts`、`split-order.spec.ts`、`order-dishes.smoke.spec.ts`**
...
- [ ] **Step 1: 保留 `RecallPage` 作为 facade，公开搜索、打开详情、进入编辑、进入支付、作废订单等稳定能力**
- [ ] **Step 2: 将筛选条件、手动搜索、详情弹窗读取、作废弹窗交互拆到聚焦 section/dialog 文件**
- [ ] **Step 3: 去掉详情弹窗与作废流程中的猜测式 locator 链，只保留真实 DOM 契约与必要的互斥作用域封装**
- [ ] **Step 4: 统一 `openX`、`readX`、`closeX`、`voidX` 的命名与后置条件，避免名字轻、内部策略重**
- [ ] **Step 5: 回跑 `recall-*.spec.ts`、`recall-order-details.smoke.spec.ts`、`recall-search.smoke.spec.ts`、`dine-in-recall-consistency.smoke.spec.ts`**
...
- [ ] **Step 1: 把同页动作统一收敛为 `Promise<void>`，跨页动作统一返回目标 page，读取动作统一返回明确数据模型**
- [ ] **Step 2: 清理 `clickX`、`openX`、`enterX`、`saveX`、`readX`、`expectX` 的命名混用，名字必须直接表达动作层级**
- [ ] **Step 3: 给跨页动作补最小加载校验约定，并保持 page 层不掺杂业务选择策略**
- [ ] **Step 4: 回跑点单、Recall、库存、支付、分单与 takeout 相关用例，确认 API 语义调整不改变业务行为**
...
- [ ] **Step 1: 统计各 flow 的真实调用入口，确定保留“decorated class methods + 薄包装函数”还是“函数优先、类内聚不导出”的单一模式**
- [ ] **Step 2: 删除未被使用、只制造阅读分叉的一比一重复导出，同时保留报告步骤可见性**
- [ ] **Step 3: 批量更新测试与 fixture 的 import，确保团队只面对一种推荐调用方式**
- [ ] **Step 4: 回跑 `tests\smoke`、`tests\py-migrate` 与受影响 e2e，确认 flow 清理不破坏业务编排**
...
- [ ] **Step 1: 把 locator 契约、命名规则、返回值规则、page/flow 边界、后置条件约定写入项目文档**
- [ ] **Step 2: 在 `AGENTS.md` 中补充“避免 locator 猜测链”“统一返回值语义”“同页动作不默认 `return this`”等规则**
- [ ] **Step 3: 为 `report-noise-guard.spec.ts` 增加结构性护栏，如禁止新增超长 `.or(...)` 链、限制热点 page 文件重新膨胀**
- [ ] **Step 4: 全量回跑 `report-noise-guard.spec.ts`、关键 e2e、关键 smoke，记录剩余非本次重构问题并单独归档**
