...
在 `tests/py-migrate/order-page-regression.spec.ts` 新建一个独立的 Playwright Test 回归文件，实现覆盖矩阵中 5 条 `待补断言` 和 9 条 `可直接实现` 场景。新文件只使用仓库已经确认的真实 Page、Flow、fixture 与 API 契约，不混入 48 条仍需录制的场景。
...
| 折扣与备注 | 7 | POS-42886 | 待补断言 | 新增目标用例，保留不同 Jira 的 POS-23204 |
| 折扣与备注 | 8 | POS-42888 | 待补断言 | 新增目标用例，保留不同 Jira 的 POS-24394 |
| 折扣与备注 | 28 | POS-28674 | 可直接实现 | 新增改价后单菜 50% 折扣用例 |
...
`tests/py-migrate/order-page-regression.spec.ts` 是 14 条场景的唯一入口，包含四个中文 `describe`：
...
- `tests/py-migrate/split-order-operation.spec.ts`
  - 保留 POS-23204 与 POS-24394；它们分别覆盖清空折扣和复制订单，不是 POS-42886/POS-42888 的等价重复。
...
复用并扩展 `test-data/order-service.ts`。固定菜品、菜单、数量、分单金额、折扣比例和期望整数分金额放在测试数据层；spec 只保留场景编排和断言。动态订单标识由现有保存响应或 Recall 读取能力产生，不在测试体内使用 `Date.now()`。
