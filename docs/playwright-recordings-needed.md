...
订单操作回归当前共有 **7 条 skipped**：其中 **6 条需要录制**，归并为 **2 组录制请求**（报表 Fee/Unpaid 5 条，POS-27242 1 条）；POS-32954 因“POS NG 不适用”保留 skip 但不需要录制。点单页面提示词另新增 **47 条**一对一录制请求。
...
- 来源编号：提示词 2；业务背景：验证「POS-15605 点单页面菜单--组 中文展示」（菜单与语言），矩阵状态为「需要录制」。
- Jira：`POS-15605`。
- 已知前置：业务域为「菜单与语言」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 录制提交要求：本场景单独提交为一个三反引号 `ts` 代码块，保留录制器生成的原始定位与关键请求；若实际结果与预期不同，请附最终页面截图并注明实际结果。

### ORDER-PAGE-002：提示词 3 POS-15641 点单页面菜单--类--组下多个类时，进行类切换

- 来源编号：提示词 3；业务背景：验证「POS-15641 点单页面菜单--类--组下多个类时，进行类切换」（菜单与语言），矩阵状态为「需要录制」。
- Jira：`POS-15641`。
- 已知前置：业务域为「菜单与语言」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 4；业务背景：验证「Seperate the same dishes开关开启，订单送厨后编辑页面，选择菜品，点击加1，存单后检查订单上的税率正确」（送厨与合并菜），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：业务域为「送厨与合并菜」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 5；业务背景：验证「后台点单前确认客户信息，客户姓名必填，电话必填，点支付按钮后弹出输入电话号码和客户姓名的弹框」（客户信息），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：业务域为「客户信息」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 6；业务背景：验证「没有删菜权限的用户点单后删菜，弹出权限提示框提示没有权限，输入正确的密码可正常删菜」（权限与 Void），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：业务域为「权限与 Void」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 10；业务背景：验证「open Food 点单，不选择任何税，可以成功完成付款」（Open Food），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：业务域为「Open Food」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 11；业务背景：验证「连续创建两个不输入姓名的pick up订单，修改其中一个食客姓名，另一个不受影响」（客户信息），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：业务域为「客户信息」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 13；业务背景：验证「POS-15737 点单界面菜单--类，类的中文展示」（菜单与语言），矩阵状态为「需要录制」。
- Jira：`POS-15737`。
- 已知前置：业务域为「菜单与语言」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 15；业务背景：验证「POS-15759 点单界面菜单--option 创建类的option,包含二级option，点单时选择了类的option，未选二级option」（Option），矩阵状态为「需要录制」。
- Jira：`POS-15759`。
- 已知前置：业务域为「Option」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 16；业务背景：验证「POS-15760 点单界面菜单--option 创建菜的option,包含二级option，点单时选择了菜的option，未选二级option」（Option），矩阵状态为「需要录制」。
- Jira：`POS-15760`。
- 已知前置：业务域为「Option」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 17；业务背景：验证「POS-15761 点单界面菜单--option 创建菜的option，点单时选择了菜的option」（Option），矩阵状态为「需要录制」。
- Jira：`POS-15761`。
- 已知前置：业务域为「Option」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 18；业务背景：验证「POS-15762 点单界面菜单--option 创建菜的option，点单时未选择菜的option」（Option），矩阵状态为「需要录制」。
- Jira：`POS-15762`。
- 已知前置：业务域为「Option」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 19；业务背景：验证「POS-15763 点单界面菜单--option 创建菜的option,包含二级option，点单时选择了菜的option，和二级option」（Option），矩阵状态为「需要录制」。
- Jira：`POS-15763`。
- 已知前置：业务域为「Option」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 27；业务背景：验证「test_open_food_keyboard_multi_language」（Open Food），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：业务域为「Open Food」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 29；业务背景：验证「POS-30575：delivery点单，输入用户信息，进入点单页面点击info,info信息预输入的一致」（客户信息），矩阵状态为「需要录制」。
- Jira：`POS-30575`。
- 已知前置：业务域为「客户信息」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 30；业务背景：验证「POS-31045：选择的套餐子菜包含多个option，点单页面连续删除option正常」（Modify），矩阵状态为「需要录制」。
- Jira：`POS-31045`。
- 已知前置：业务域为「Modify」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 31；业务背景：验证「POS-30762：切换pos菜单模式后，搜索检查，默认搜索菜品 Broccoli Garlic Sauce」（菜单与搜索），矩阵状态为「需要录制」。
- Jira：`POS-30762`。
- 已知前置：业务域为「菜单与搜索」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 32；业务背景：验证「POS-31662：点单页面，选择菜品，点击modify，添加任意global option，点击add，新增数量成功，页面右侧继续展示modify页面」（Modify），矩阵状态为「需要录制」。
- Jira：`POS-31662`。
- 已知前置：业务域为「Modify」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 33；业务背景：验证「POS-31663：点单页面，选择菜品，点击modify，添加任意global option，点击count，增加数量，新增数量成功，页面右侧继续展 示modify页面，点击count修改数量为0，点单列表不再展示该global option，页面右侧继续展示modify页面」（Modify），矩阵状态为「需要录制」。
- Jira：`POS-31663`。
- 已知前置：业务域为「Modify」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 34；业务背景：验证「POS-31664：点单页面，选择菜品，点击modify，添加任意global option，点击count，增加数量为2，页面右侧继续展示modify页 面，点击reduce，页面右侧继续展示modify页面，减少至0，点单列表不再展示该global option，页面右侧继续展示modify页面」（Modify），矩阵状态为「需要录制」。
- Jira：`POS-31664`。
- 已知前置：业务域为「Modify」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 35；业务背景：验证「下单页和订单详情卡片上展示用户信息」（客户信息），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：业务域为「客户信息」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 36；业务背景：验证「POS-33447：Search Menu设置关闭，进入点单页面，页面不再展示搜索输入框，页面展示无违和 POS-33456：Search Menu设置开启，recall进入订单编辑页面，页面展示搜索输入框，可正常搜索，默认搜索菜品Broccoli Garlic Sauce」（菜单与搜索），矩阵状态为「需要录制」。
- Jira：`POS-33447, POS-33456`。
- 已知前置：业务域为「菜单与搜索」；使用源提示词指定的既有普通菜、菜单/分类、option、厨房菜或临时数据；所需订单类型、员工/权限、后台配置按完整录制路径准备，场景结束后恢复。
- 请从 POS 首页开始录制：
  1. 最小录制路径：从 POS 首页读取 Search Menu 原值，关闭后刷新并进入堂食无桌，断言搜索入口/输入框不存在后保存可编辑订单；返回首页开启 Search Menu 并刷新，从 Recall 精确打开该订单 Edit，断言搜索输入框出现，搜索并选择 Broccoli Garlic Sauce，最后恢复原开关；当前缺失的准确 UI 动作是“Search Menu 开关关闭/开启，以及 Recall 编辑页搜索框隐藏/显示和搜索”；
  2. 逐一展示上述完整路径中的中间弹窗、控件状态、页面转场、配置生效与恢复动作，不得省略标题对应的业务步骤；
  3. 在最终断言所在页面读取并保留以下结果：`tests/py-migrate/order.service.spec.ts` 分别断言关闭态不可见、开启态可见且精确结果文本。
- 请保留证据：配置项名称、ID、旧值/关/开值及查询/更新/恢复请求，搜索入口、输入、结果卡和 Recall Edit 稳定 DOM，订单号与菜单响应；同时保留目标元素原始的 data-testid、role、label、可见文本，以及影响最终结果的关键网络请求、配置旧值/新值和恢复结果。
- 当前阻塞：Search Menu 开关关闭/开启，以及 Recall 编辑页搜索框隐藏/显示和搜索。
...
- 来源编号：提示词 40；业务背景：验证「POS-34106 点单页面，礼品卡实体卡新增页面，手机号输入框，手机号输入正确，可新建卡成功」（礼品卡），矩阵状态为「需要录制」。
- Jira：`POS-34106`。
...
- 来源编号：提示词 41；业务背景：验证「POS-34873 用户无Edit Order->Void Printed Item权限，删除hold的菜品（reduce删除），输入有权限的密码，可删除成功 系统中在staff中预置了用户1（非boss权限），以防与其他用户冲突」（权限与 Void），矩阵状态为「需要录制」。
- Jira：`POS-34873`。
...
- 来源编号：提示词 42；业务背景：验证「POS-35325 用户无Edit Order->Void Printed Item权限，删除延迟送厨的菜品（count数量减少），输入有权限的密码，可删除成功 系统中在staff中预置了用户1（非boss权限），以防与其他用户冲突」（权限与 Void），矩阵状态为「需要录制」。
- Jira：`POS-35325`。
...
- 来源编号：提示词 43；业务背景：验证「POS-34895 设置不自动合并相同菜，点单页面选中相同菜，1菜1行，不合并」（送厨与合并菜），矩阵状态为「需要录制」。
- Jira：`POS-34895`。
...
- 来源编号：提示词 44；业务背景：验证「POS-34903 设置相同菜合并显示(状态一致的)，已经存在不含有option的菜（已送厨），新加菜分开显示」（送厨与合并菜），矩阵状态为「需要录制」。
- Jira：`POS-34903`。
...
- 来源编号：提示词 45；业务背景：验证「POS-34910 设置相同菜合并显示(包含已送厨），相同菜已送厨，编辑后继续加相同菜，加入到已送厨的一行， 保持合并展示，显示出 X In Kitchen，菜品字体红色」（送厨与合并菜），矩阵状态为「需要录制」。
- Jira：`POS-34910`。
- 已知前置：可送厨房普通菜、可用厨房链路，以及两项配置入口的更新/恢复权限。
- 请从 POS 首页开始录制：
  1. 最小录制路径：从 POS 首页分别读取并暂存源步骤中的两项配置，先执行“相同菜品包含已送厨时也合并到同一行”，再执行“相同菜品分开展示开关”，记录每次动作的独立请求响应并刷新；进入堂食无桌添加目标菜、送厨并保存订单号，从 Recall 精确打开同单编辑后再次添加同一菜，读取合并行数量、`X In Kitchen` 文案和菜名字体颜色，保存回首页，finally 执行源步骤的“不自动合并”并按相反顺序恢复两项原值后刷新。前置数据：可送厨房普通菜、可用厨房链路，以及两项配置入口的更新/恢复权限。当前缺失的准确 UI 动作是“逐项执行包含已送厨合并与分开展示开关，并读取合并行厨房数量标记/红色样式”；必须由网络证据判断两项动作的真实配置关系；
  2. 逐一展示上述完整路径中的中间弹窗、控件状态、页面转场、配置生效与恢复动作，不得省略标题对应的业务步骤；
  3. 在最终断言所在页面读取并保留以下结果：`tests/py-migrate/order.service.spec.ts` 断言只有 1 条同名行、总数量正确、厨房数量标记出现、菜名字体为产品定义红色，并确认配置恢复。
- 请保留证据：两项入口 DOM，各自准确名称、ID、类型、旧值/新值、请求响应与刷新结果，Send/Recall Edit、合并菜行、`X In Kitchen` 节点和计算样式 DOM，保存/送厨响应中的 itemId、总数量和厨房数量；同时保留目标元素原始的 data-testid、role、label、可见文本，以及影响最终结果的关键网络请求、配置旧值/新值和恢复结果。
- 当前阻塞：逐项执行包含已送厨合并与分开展示开关，并读取合并行厨房数量标记/红色样式。
...
- 来源编号：提示词 46；业务背景：验证「POS-34842 Automatically redirect after reduce items开关关闭，相连两个菜属于不同类别，当前选中菜品减到没有时，停留 原category」（数量），矩阵状态为「需要录制」。
- Jira：`POS-34842`。
...
- 来源编号：提示词 47；业务背景：验证「POS-33186 点单，修改菜的数量为小于1的小数，减菜正常」（数量），矩阵状态为「需要录制」。
- Jira：`POS-33186`。
- 已知前置：支持小数数量的普通菜。源标题要求“小于 1”，但源步骤写 `num=1.25`，两者业务边界冲突。
- 请从 POS 首页开始录制：
...
- 当前阻塞：输入真实小于 1 的数量后执行 Reduce，确认按步长减少还是直接移除。
...
- 来源编号：提示词 48；业务背景：验证「POS-33241 点单，包含小数数量的菜，拖拽分单正常，母单和子单菜的小数数量展示正常，价格正确」（分单与合单），矩阵状态为「需要录制」。
- Jira：`POS-33241`。
...
- 来源编号：提示词 50；业务背景：验证「POS-33600 点单，选择指定价格的菜，修改菜的数量为指定小数，保存订单成功」（数量），矩阵状态为「需要录制」。
- Jira：`POS-33600`。
- 已知前置：支持小数数量与改价的目标菜、追加菜的整数分菜单价，以及确认产品金额舍入规则的业务依据。`650分 × 2.55 = 1657.5分` 产生半分，仓库没有产品每行/整单舍入口径。
- 请从 POS 首页开始录制：
...
- 当前阻塞：矩阵最近验证确认：确认 Page API 入参为 6.50 美元，但 2.55 数量产生半分且无产品舍入契约；尚缺上述完整路径对应的真实 UI、网络或配置契约。
...
- 来源编号：提示词 52；业务背景：验证「POS-35129 点单，选择指定价格的菜，修改菜的数量为指定小数，保存订单成功」（数量），矩阵状态为「需要录制」。
- Jira：`POS-35129`。
- 已知前置：支持小数数量的普通菜、候选指定价格及可保存回查的 To Go 订单。标题/业务目标明确要求“指定价格”和“保存成功”，但详细步骤只改数量并读取点单行。
- 请从 POS 首页开始录制：
...
- 当前阻塞：矩阵最近验证确认：按 47 同样保守处理标题/业务目标与详细步骤冲突；尚缺上述完整路径对应的真实 UI、网络或配置契约。
...
- 来源编号：提示词 53；业务背景：验证「POS-35660 开启相同菜合并展示开关，点单，修改菜的数量为小数，给菜添加调味正常」（Modify），矩阵状态为「需要录制」。
- Jira：`POS-35660`。
...
- 来源编号：提示词 54；业务背景：验证「POS-22640 自定义类型-POS点单点击自定义类型 Delivery，点单，打单成功」（自定义类型与打印），矩阵状态为「需要录制」。
- Jira：`POS-22640`。
...
- 来源编号：提示词 55；业务背景：验证「POS-36286 POS首页delivery下单，添加用户信息进入订单页面，点击退出直接退出点单页」（客户信息），矩阵状态为「需要录制」。
- Jira：`POS-36286`。
- 已知前置：无同名待处理订单且可追踪本次唯一客户数据；库存 To Go 用例调用 `exitOrderPage` 不能证明 Delivery 填写客户/地址后的退出行为，现有方法通过 Back/iframe/`#odBack` 候选定位、只在瞬时可见时点确认且不等待首页后置，不能作为稳定 DOM 契约。
- 请从 POS 首页开始录制：
...
- 当前阻塞：Delivery 客户信息进入点单页后使用专属退出路径，并处理实际确认分支。
...
- 来源编号：提示词 56；业务背景：验证「POS-36255 菜名名称与number（如均为AA）相同时，点单页面搜索只能搜到一个AA」（菜单与搜索），矩阵状态为「需要录制」。
- Jira：`POS-36255`。
- 已知前置：可写入并清理的菜单分类；`apiSetup.saleItem.create/delete` 已有通用 CRUD，但 `SaleItemApiRequest` 未建模 number 字段，`OrderDishesMenuSection.searchAndClickDish` 只用候选 locator 找首个结果并点击，不能证明唯一结果。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 57；业务背景：验证「POS-37804 无note权限用户，套餐子菜加note出权限提示」（权限与 Void），矩阵状态为「需要录制」。
- Jira：`POS-37804`。
...
- 来源编号：提示词 58；业务背景：验证「必选类功能优化，未满足条件时无法提交，且自动跳转」（菜单配置），矩阵状态为「需要录制」。
- Jira：无。
...
- 来源编号：提示词 59；业务背景：验证「category未勾选“限制折扣”导致该category下所有菜品可以参与整单按比例加收」（菜单配置），矩阵状态为「需要录制」。
- Jira：无。
...
- 来源编号：提示词 60；业务背景：验证「新UI点单页面，category正常展示配置的POS NAME」（菜单配置），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：可创建/删除分类与菜品的 API 权限；`apiSetup.category.create` 虽接受通用 overrides，但 `CategoryApiRequest` 未建模 posName，现有 `switchMenuCategory` 只按给定文本点击且没有当前类别窄读取，不能证明 UI 用 POS NAME 而非 name/displayName。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 61；业务背景：验证「套餐的子菜支持改价」（套餐），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：可改价套餐及员工改价权限。
- 请从 POS 首页开始录制：
...
- 来源编号：提示词 62；业务背景：验证「按菜分单后子单打折，打折界面整单金额检查」（分单与折扣），矩阵状态为「需要录制」。
- Jira：无。
...
- 来源编号：提示词 63；业务背景：验证「自定义类型-报表显示Report-Overview-预览显示自定义类型的报表数据-数据正确」（报表），矩阵状态为「需要录制」。
- Jira：无。
- 已知前置：目标自定义订单类型、云报表账号/服务和可恢复配置；`ReportPage` 当前为空，`HomePage.enterReport` 只点击入口，自定义类型入口也缺失。“打印提示信息”疑似源步骤误写，但在产品确认前必须保留为独立中间断言，不能替代或合并最终 Overview 数据断言。
- 请从 POS 首页开始录制：
...
- 当前阻塞：启用后读取 amount/打印提示、刷新，再进入 frame 选择类型并读最终报表。
...
- 来源编号：提示词 64；业务背景：验证「系统语言为中文，点单搜索框输入菜的首字母返回对应的菜」（语言与搜索），矩阵状态为「需要录制」。
- Jira：无。
...
- 来源编号：提示词 65；业务背景：验证「套餐设置display all one time，子菜不可重复选，规则设置max，点单保存combo后，可正常编辑，修改子菜」（套餐），矩阵状态为「需要录制」。
- Jira：无。
...
- 来源编号：提示词 66；业务背景：验证「combo子菜没有option，选择子菜返回主菜可正常选择option」（套餐），矩阵状态为「需要录制」。
- Jira：无。
